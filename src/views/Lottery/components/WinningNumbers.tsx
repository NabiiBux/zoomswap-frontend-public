...
const WinningNumbers: React.FC = () => {
  ...
  const noLastest = [0, 0, 0, 0]

  return (
    <CardWrapper style={{ display: winNumbers.toString() === noLastest.toString() ? 'none' : 'block' }}>
      <Card>
        <CardBody>
            ...
            <StyledCardHeader>
              <Title>
                {account && lotteryHasDrawn
                  ? `🥳${TranslateString(570, 'Winning Numbers')}🥳`
...
const TicketNumberBox = styled.div`
  border-radius: 12px;
  background-image: url('/images/sign-bunny.png');
  background-size: 100%;
  background-repeat: no-repeat;
  color: rgba(238, 42, 255, 1);
  font-size: 20px;
  font-weight: 900;
  margin: 10px;
  margin-bottom: 7px;
